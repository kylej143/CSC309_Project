...
export default function codetemplates() {
    ...
    const create = async (val: React.FormEvent) => {
        val.preventDefault();
        const tk = localStorage.getItem("accessToken");
        if (tk) {
            try {
                const r = await fetch("/api/user/code_templates", {
    ...
    const update = async (val: React.FormEvent) => {
        val.preventDefault();
        if (!st) {
            return;
        } else {
            ...
            if (tk) {
                try {
                    const r = await fetch(`/api/user/code_templates?id=${st.id}`, {
    ...
    const fork2 = async (val: React.FormEvent) => {
        val.preventDefault();
        if (!ft) {
            return;
        } else {
            const tk = localStorage.getItem("accessToken");
            try {
                const r = await fetch(`/api/user/code_templates?fork=true&id=${ft.id}`, {
                    method: "POST", headers: { "Content-Type": "application/json", ...(tk && { Authorization: `Bearer ${tk}` }) },
                    body: JSON.stringify({
                        title: ft.title,
